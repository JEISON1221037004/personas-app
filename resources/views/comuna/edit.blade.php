  ...
  <h1>Edit Comune</h1>
    <form method="POST" action="{{ route('comunas.update', ['comuna' => $comuna->comu_codi]) }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="id" aria-describedby="codigoHelp" name="id" value="{{ $comuna->comu_codi }}" disabled="disabled">
            <div id="codigoHelp" class="form-text">Comune Id.</div>
        </div>
        <div class="mb-3">
            <label for="name" class="form-label">Comune</label>
            <input type="text" class="form-control" id="name" placeholder="Comune name" name="name" value="{{ $comuna->comu_nomb }}" required>
        </div>
